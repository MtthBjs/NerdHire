#include <iostream>
#include "Common.h"

Common::Common()
{
}

Common::~Common()
{
}

u8 confusion[512]={
};

u32 diffusion[32]={
0xf26cb481,0x16a5dc92,0x3c5ba924,0x79b65248,0x2fc64b18,0x615acd29,0xc3b59a42,0x976b2584,
0x6cf281b4,0xa51692dc,0x5b3c24a9,0xb6794852,0xc62f184b,0x5a6129cd,0xb5c3429a,0x6b978425,
0xb481f26c,0xdc9216a5,0xa9243c5b,0x524879b6,0x4b182fc6,0xcd29615a,0x9a42c3b5,0x2584976b,
0x81b46cf2,0x92dca516,0x24a95b3c,0x4852b679,0x184bc62f,0x29cd5a61,0x429ab5c3,0x84256b97};

void Common::PrintU8ArrayWithName(u8 array[], unsigned int size, const char* name, bool with_endline)
{
	if(with_endline)
	{
		for(unsigned int i = 0 ; i < size ; ++i)
		{
			std::cout << name << "[" << i << "]=" << (unsigned int)array[i] << std::endl;
		}	
	}
	else
	{
		std::cout << name << std::endl;
		for(unsigned int i = 0 ; i < size ; ++i)
		{
			std::cout << (unsigned int)array[i] << ",";
		}
		std::cout << std::endl;
	}
}

void Common::PrintU8ArrayWithName(std::vector<u8> vec, unsigned int size, const char* name, bool with_endline)
{
    u8 array[size];
    for(unsigned i = 0 ; i < size ; ++i) array[i] = vec[i];
    PrintU8ArrayWithName(array, size, name, with_endline);
}
    

bool Common::IsValueReachableInConfusion(u8 val)
{
    static std::unordered_set<u8> *confusion_values;
	if(confusion_values == nullptr)
	{
		confusion_values = new std::unordered_set<u8>();
		for(u32 i = 0 ; i < 512 ; ++i)
		{
			if(confusion_values->find(confusion[i]) == confusion_values->cend())
			{
				confusion_values->insert(confusion[i]);
			}
		}
	}
	
	return confusion_values->find(val) != confusion_values->cend();
}
